    ...
    <div class="col-md-6 col-md-offset-1 text-center" style="margin-top:-340px">
            ...
            <div class="container">
              ...
              <div class="row colored">
                <h2>Todo List</h2>
                <?php foreach ($todo_sess as $key => $value) { ?>
                
                <form method="POST" action="">
                  <h3>Edit Todo</h3><input type="text" name="todo_title" value="<?= $value->todo_title ?>">
                  <select name="todo_assign" class="form-control">
                      <option value='Unassigned'>Unassigned</option>
                      <?php 
                      foreach ($users as $key => $ff) {
                        if ($todo_sess->todo_assign == $fff->fullname) {
                           echo "<option value='".$ff->fullname."' selected>".$ff->fullname."</option>";
                        }
                        else{
                      ...
                      foreach ($user as $key => $fff) {
                        if ($todo_sess->todo_assign == $fff->fullname) {
                           echo "<option value='".$fff->fullname."' selected>".$fff->fullname."</option>";
                        }
                        else{
                  ...
                  <input type="date" class="form-control" name="due_date" value="<?= $value->due_date ?>">
                  <input type="text" name="todo_id" hidden value="<?= $value->todo_id ?>">
                  <input type="text" name="todo_project" hidden value="<?= $value->todo_project ?>">
                  <br/><br/>
